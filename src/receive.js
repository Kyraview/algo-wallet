...
export default function Receive() {
    ...
    return(
        enableAddress?<div className='row' style={{height:'100vh', background:'#333'}}>
        ...
        <QRCode size="150" value={address} style={{background:'white'}}/>
        </div>
        </div>
        <div className="col">
        <CopyToClipboard text={address}>
        <p id="blinkAddress" onClick={blink} style={{wordBreak:'break-all', width:'80%', fontSize:'100%', transform:'translateY(50%)', position:'absolute', padding:'0 10px 0 0', color:'white', textAlign:'center'}}>
            {address}
            <img alt='' onClick={blink} style={{width:'10%', marginLeft:'5px', cursor:'pointer'}} src={copyIcon}/>
        </p>
        </CopyToClipboard>
        </div></div>:
        <div style={{height:'100vh', display:'flex', flexDirection:'column', justifyContent:'center', background:'#333'}}>
